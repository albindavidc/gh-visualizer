...
// Vercel serverless functions often struggle with importing outside of the api directory 
// if the build configuration isn't perfectly set up for it, especially when mixing ts-node and esbuild.
// The best way to ensure absolute stability on Vercel is to ensure the generator logic is self-contained 
// or the imports are resolved via Vercel's bundler.
// Let's actually inline the svgGenerator logic into the api/graph.ts file so it's a completely self-contained function.
