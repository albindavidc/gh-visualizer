...
(async () => {
  ...
  const getColors = async () => {
    return page.evaluate(() => {
      ...
      const headerText = el.querySelector('span');
      return {
        headerDivBg: window.getComputedStyle(el).backgroundColor,
        headerTextCol: window.getComputedStyle(headerText).color,
        graphContainerBg: window.getComputedStyle(el.parentElement.parentElement).backgroundColor,
      };
    });
  };
