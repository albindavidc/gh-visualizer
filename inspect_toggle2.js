...
(async () => {
  ...
  await page.waitForFunction(() => {
    return Array.from(document.querySelectorAll('div')).some(d => d.innerText && d.innerText.includes('LeetCode Stats'));
  }, { timeout: 15000 });

  const getStyle = async () => {
    return page.evaluate(() => {
      const divs = Array.from(document.querySelectorAll('div'));
      const textDiv = divs.find(d => d.innerText && d.innerText.trim() === 'LeetCode Stats' && d.children.length === 0);
